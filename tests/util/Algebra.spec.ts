...
import { Functor, Applicative, Monad } from 'util/Algebra';
...
export function functorLaws(f: (x: any) => Functor<any>): void {
	it('should satisfy functor identity', function() {
		expect(f(42).map(id)).to.be.deep.equal(f(42));
	});

	it('should satisfy functor composition', function() {
		expect(f(42).map(compose(gn, fn))).to.be.deep.equal(f(42).map(gn).map(fn));
	});
}
