...
import {
	UpdateProductionBlockTypes,
	UpdateProductionBlockAction
} from "./actions/updateProductionBlock";
import {
	FetchRatesActionTypes,
	FetchRatesAction,
	fetchRatesSuccess,
	fetchRatesFail
} from "./actions/fetchRates";
import {
	RemoveAllSelectedTypes,
	RemoveAllSelectedAction
} from "./actions/removeAllSelected";
...
export const productsReducer: LoopReducer<ProductsState, ProductsAction> = (
	...
	switch (action.type) {
		...
		case FetchRatesActionTypes.Fail: {
			return {
				...state,
				productChunks: Response.Error(
					"There was a problem fetching production rate data."
				)
			};
		}
		...
		case ToggleActionTypes.ToggleSelectedProduct: {
			const indexOfProduct = productListContainsProduct(
				state.productBlocks.map(b => b.product)
			)(action.product);
		...
		case UpdateProductionBlockTypes.UpdateProductionBlock: {
			const productionBlockLens = compose(
				lensProp("productBlocks"),
				action.blockPath
			) as Lens;
